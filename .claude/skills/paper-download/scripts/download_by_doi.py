"""
Helper script to download a single DOI's PDF (and supplementary PDFs) using
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Download a single DOI PDF (and supplementary PDFs) via auto_paper_download."
        )
    )
    parser.add_argument(
        "--doi",
        required=True,
        help="DOI to download (e.g., 10.1038/s41586-020-2649-2)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def main(argv: list[str] | None = None) -> None:
    ...
    doi = args.doi.strip()
    if not doi:
        raise SystemExit("DOI must be non-empty.")
